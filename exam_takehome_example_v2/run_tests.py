'''
    ...
    The optional argument --verbose determines the amount of output:
    ...
    To abort after first failed test use --abort.
'''
...
def run_test(program, test_in, test_out, verbose, timeout=TIMEOUT):
    ...
    if not ok and verbose >= 2 or verbose >= 3:
        ...
        if not ok:
            log('Received output', answer[:len(correct) + 3])
            if not answer:
                log(lines='(none)')
            if len(answer) > len(correct) + 3:
                truncated = True
            if truncated:
                log(lines='(output truncated)')
            if error:
                log('Error', error[-5:])
        log(indent=-2)
...
def test_all(programs=None, verbose=1):
    ...
    for program, (passed, tests) in results.items():
        log(f'{program:{width}} {passed:2}/{tests}')
        total_passed += passed
        total_tests += tests
    final_result = f'{"Total":{width}} {total_passed:2}/{total_tests}'
