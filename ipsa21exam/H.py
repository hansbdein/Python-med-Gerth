...
def print_stars(stars, vertical=False, highscore=False,alphabetically=False):
    
    #names=[name for name,star in stars]
    length=max(len(name) for name,star in stars)+1
    starmax=max([star for name,star in stars])
    L=[name+
       '_'*(length-len(name))+ 
       '*' * int(star) 
    ...
    if not vertical:
        [print(txt) for txt in L]
        
    
    if vertical:
        maxlen=max([len(x) for x in L])
        
        bs=[txt+'_'*(maxlen-len(txt)) for txt in L]
        
        i=0
        for zipped in reversed(list(zip(*bs))):
        ...
        i=0
        for zipped in zip(*bs):
            
            print('_'.join(zipped))
            i+=1
            if i>=length-1:
                break
    #if not vertical or highscore or alphabetically:
